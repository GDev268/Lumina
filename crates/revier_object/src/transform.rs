...
impl Transform {
    pub fn get_mat4(&self) -> glam::Mat4 {
        let c1 = self.rotation.x.cos();
        let s1 = self.rotation.x.sin();
        let c2 = self.rotation.y.cos();
        let s2 = self.rotation.y.sin();
        ...
        return glam::mat4(
            glam::vec4(
                self.scale.x * (c2 * c3),
                self.scale.x * (c1 * s3 + s1 * s2 * c3),
                self.scale.x * (s1 * s3 - c1 * s2 * c3),
                0.0,
            ),
            glam::vec4(
                self.scale.y * (-c2 * s3),
                self.scale.y * (c1 * c3 - s1 * s2 * s3),
                self.scale.y * (s1 * c3 + c1 * s2 * s3),
                0.0,
            ),
            glam::vec4(
                self.scale.z * (s2),
