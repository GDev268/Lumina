...
impl Loader {
    ...
    pub fn load_file(&mut self, file_path: String) {
        ...
        loop {
            ...
            if file.read_exact(&mut directory_name_len_bytes).is_err() {
                break;
            }

            let directory_name_len = u32::from_le_bytes(directory_name_len_bytes) as usize;

            let mut dir_name_bytes = vec![0u8; directory_name_len];
            file.read_exact(&mut dir_name_bytes).unwrap();
            ...
            let directory_name = String::from_utf8_lossy(&decrypted_dir_name).into_owned();

            let mut num_files_bytes = [0u8; 4];
            file.read_exact(&mut num_files_bytes).unwrap();
            ...
            for _ in 0..num_files {
                ...
                file.read_exact(&mut file_type_byte).unwrap();
                file.read_exact(&mut name_size_bytes).unwrap();
                file.read_exact(&mut content_size_bytes).unwrap();
                ...
                let file_type = match file_type_byte[0] {
                    ...
                    _ => {
                        println!("ERROR: Couldn't find an file type!");
                        LuminaFileType::None
                    }
                };

                let name_size = u32::from_le_bytes(name_size_bytes) as usize;
                let content_size = u32::from_le_bytes(content_size_bytes) as usize;

                let mut name_bytes = vec![0u8; name_size];
                file.read_exact(&mut name_bytes).unwrap();

                let decrypted_file_name = decrypt(&name_bytes, LUMINA_KEY)
                    .map_err(|e| io::Error::new(io::ErrorKind::Other, e))
                    .unwrap();
                let file_name = String::from_utf8_lossy(&decrypted_file_name).into_owned();
            ...
            self.directories.insert(directory_name.clone(),LuminaDirectory {
                name: directory_name,
                files,
            });
        }
