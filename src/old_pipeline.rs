...
pub struct OLD_PipelineConfiguration {
...
impl OLD_PipelineConfiguration {
    pub fn default(width:u32,height:u32) -> Self {
        let mut pipe_config = OLD_PipelineConfiguration::default_create(width, height);

        pipe_config.color_blend_info.p_attachments = &pipe_config.color_blend_attachment;
        pipe_config.viewport_info.p_viewports = &pipe_config.viewport;
        pipe_config.viewport_info.p_scissors = &pipe_config.scissor;
    ...
    pub fn default_create(width:u32,height:u32) -> Self{
        ...
        let mut viewport:vk::Viewport = vk::Viewport::default();
        viewport.x = 0.0;
        viewport.y = 0.0;
        ...
        let mut color_blend_attachment = vk::PipelineColorBlendAttachmentState {
        ...
        color_blend_info.p_attachments = std::ptr::null();
...
impl OLD_Pipeline {
    pub fn new(
        ...
        vert_file_path: &str,
        frag_file_path: &str,
        pipeline_config: OLD_PipelineConfiguration,
    ) -> Self {
        assert!(
        ...
        let mut pipeline = OLD_Pipeline::default();

        let vert_code = OLD_Pipeline::read_file(vert_file_path);
        let frag_code = OLD_Pipeline::read_file(frag_file_path);

        pipeline.vert_shader_module = OLD_Pipeline::create_shader_module(vert_code, device);
        pipeline.frag_shader_module = OLD_Pipeline::create_shader_module(frag_code, device);
