...
use crate::{
    components::{shapes::cube::PushConstantData, transform::Transform, model::Model},
    engine::{
        device::Device,
        scene::Scene,
    ...
    pub fn render_game_objects(&self, device: &Device, frame_info: &FrameInfo, scene: &mut Scene) {
        ...
        for (id, entity) in scene.entities.iter_mut() {
            let push: PushConstantData = if entity.has_component::<Transform>() {
                PushConstantData {
                    model_matrix: frame_info.camera.get_projection() * entity.get_mut_component::<Transform>().unwrap().get_mat4(),
                    normal_matrix: entity.get_mut_component::<Transform>().unwrap().get_normal_matrix(),
                }
            }else{
                PushConstantData {
            ...
            if entity.has_component::<Model>(){
                entity.get_mut_component::<Model>().unwrap().render(device, frame_info.command_buffer);
