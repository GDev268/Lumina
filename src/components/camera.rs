...
impl Camera {
    ...
    pub fn set_view_yxz(&mut self, translation: glam::Vec3, rotation: glam::Vec3) {
        let c3 = rotation.x.cos();
        let s3 = rotation.x.sin();
        let c2 = rotation.y.cos();
        let s2 = rotation.y.sin();
        let c1 = rotation.z.cos();
        let s1 = rotation.z.sin();
        ...
        let adjusted_position = glam::Vec3::new(translation.x, translation.y, translation.z); // Adjusted for movement along the negative Z-axis
    
        self.view_matrix = glam::Mat4::from_cols(
            u.extend(0.0),
            v.extend(0.0),
            w.extend(0.0),
            glam::vec4(-u.dot(adjusted_position), -v.dot(adjusted_position), -w.dot(adjusted_position), 0.0),
        );
        ...
        self.inverse_view_matrix = glam::Mat4::from_cols(
            ...
            v.extend(0.0),
            w.extend(0.0),
            glam::vec4(adjusted_position.x, adjusted_position.y, adjusted_position.z, 0.0),
        );
    }
